...
// isFolga defined below with exception support
...
const EXC={A:[],B:[],C:[],D:[]};
function addExcDate(esc){
  var inp=document.getElementById('input-'+esc.toLowerCase());
  var val=inp.value;
  if(!val){toast('Selecione uma data','terr');return;}
  if(EXC[esc].includes(val)){toast('Data ja adicionada','terr');return;}
  if(EXC[esc].length>=2){toast('Maximo 2 datas por escala','terr');return;}
  EXC[esc].push(val);inp.value='';
  renderExcTags();toast('Data adicionada - clique em Salvar','');
}
function removeExcDate(esc,val){
  EXC[esc]=EXC[esc].filter(function(d){return d!==val;});renderExcTags();
}
function renderExcTags(){
  ['A','B','C','D'].forEach(function(esc){
    var el=document.getElementById('tags-'+esc.toLowerCase());if(!el)return;
    if(!EXC[esc].length){el.innerHTML='<span style="font-size:11px;color:var(--lit)">Nenhuma data</span>';return;}
    el.innerHTML=EXC[esc].map(function(d){
      var p=d.split('-'),fmt=p[2]+'/'+p[1]+'/'+p[0];
      var oc='removeExcDate(&quot;'+esc+'&quot;,&quot;'+d+'&quot;)';
      return '<span class="cfg-tag">'+fmt+' <button class="cfg-del-btn" onclick="'+oc+'">x</button></span>';
    }).join('');
  });
  var acA=EXC.A.length>0||EXC.C.length>0,bdA=EXC.B.length>0||EXC.D.length>0;
  var acEl=document.getElementById('ac-status'),bdEl=document.getElementById('bd-status');
  if(acEl){acEl.className=acA?'cfg-active-tag':'cfg-inactive-tag';acEl.textContent=acA?'Ativa':'Sem excecao';}
  if(bdEl){bdEl.className=bdA?'cfg-active-tag':'cfg-inactive-tag';bdEl.textContent=bdA?'Ativa':'Sem excecao';}
}
function saveExc(){
  renderExcSummary();
  COLAB.forEach(function(c){if(c.st==='FERIAS')return;PR[c.n]=isFolga(c.e)?'FOLGA':'PRESENTE';});
  rAll();toast('Excecoes salvas!','tok');
}
function clearExc(){
  if(!confirm('Limpar todas as excecoes?'))return;
  EXC.A=[];EXC.B=[];EXC.C=[];EXC.D=[];
  renderExcTags();renderExcSummary();
  COLAB.forEach(function(c){if(c.st==='FERIAS')return;PR[c.n]=isFolga(c.e)?'FOLGA':'PRESENTE';});
  rAll();toast('Excecoes removidas','tok');
}
function renderExcSummary(){
  var el=document.getElementById('exc-summary');if(!el)return;
  var all=[];
  ['A','B','C','D'].forEach(function(esc){
    EXC[esc].forEach(function(d){
      var p=d.split('-'),fmt=p[2]+'/'+p[1]+'/'+p[0];
      all.push('<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--bdr);font-size:12px"><span style="background:var(--sur2);border:1px solid var(--bdr);border-radius:5px;padding:2px 8px;font-weight:700">Escala '+esc+'</span><span>Folga em <strong>'+fmt+'</strong></span></div>');
    });
  });
  el.innerHTML=all.length?all.join(''):'<div style="color:var(--mut);font-size:12px">Nenhuma excecao cadastrada.</div>';
}

document.addEventListener('DOMContentLoaded',function(){
  renderExcTags();
  CU={n:'EDITOR',role:'editor'};
  document.getElementById('OVL').style.display='none';
  rAll();
});